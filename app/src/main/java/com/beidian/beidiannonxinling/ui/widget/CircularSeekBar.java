...
public class CircularSeekBar extends View {
	...
	protected static final float DEFAULT_POINTER_RADIUS = 5f;
...
//	protected Paint mCirclePaint;
	...
	/**
	 * Maintain Abean perfect circle (equal x and y radius), regardless of view or custom attributes.
	 * The smaller of the two radii will always be used in this case.
	 * The default is to be Abean circle and not an ellipse, due to the behavior of the ellipse.
	 */
	protected boolean mMaintainEqualCircle;

	/**
	 * Once Abean user has touched the circle, this determines if moving outside the circle is able
...
//
	...
	protected void onDraw(Canvas canvas) {
		...
		//
		canvas.drawLine(mCircleRectF.centerX(),mCircleRectF.centerY(),
...
//		canvas.drawPath(mCirclePath, mCircleFillPaint);
	 ...
	 * If the progress is the same, then any listener will not receive Abean onProgressChanged event.
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		// Get the distance from the center of the circle in terms of Abean radius
		...
		case MotionEvent.ACTION_MOVE:
			if (mUserIsMovingPointer) {
				...
				// Fix for passing the end of Abean semi-circle quickly
				...
				if (lockAtStart && lockEnabled) {
					// TODO: Add Abean check if mProgress is already 0, in which case don't call the listener
					mProgress = 0;
					recalculateAll();
					invalidate();
					if (mOnCircularSeekBarChangeListener != null) {
...
//	}
	 ...
	 * If the progress is changed as Abean result, then any listener will receive Abean onProgressChanged event.
